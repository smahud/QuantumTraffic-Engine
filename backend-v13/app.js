...
app.post('/api/v1/run/start', runLimiter, authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    const job = await jobManager.createJob(user, matrix, config);
    
    // Patch emitToUser function
    const jobInstance = jobManager.getJobInstance(job.jobId);
    if (jobInstance) {
      jobInstance.emitToUser = (type, payload) => {
...
/* ---------- RUNNER endpoints (NEW) ---------- */
const runnerManager = require('./lib/runnerManager');

// Get available platforms from online runners
app.get('/api/v1/runners/available-platforms', authenticateJWT, async (req, res) => {
  try {
    const platforms = runnerManager.getAvailablePlatforms();
    res.json({ 
      success: true, 
      sessionActive: true, 
      platforms: platforms,
      runnersOnline: runnerManager.getAllRunners().length
    });
  } catch (e) {
    console.error('[runners/available-platforms] error', e);
    res.status(500).json({ success: false, sessionActive: true, message: 'Server error' });
  }
});

// Get all runners status (admin only)
app.get('/api/v1/runners/status', authenticateJWT, ensureAdmin, async (req, res) => {
  try {
    const runners = runnerManager.getAllRunners();
    res.json({ 
      success: true, 
      sessionActive: true, 
      runners: runners
    ...
    console.error('[runners/status] error', e);
    res.status(500).json({ success: false, sessionActive: true, message: 'Server error' });
  }
});

// Deploy script endpoints (unprotected untuk curl/iwr)
app.get('/api/v1/runner/deploy/linux', async (req, res) => {
  try {
    const scriptPath = path.join(__dirname, '..', 'deploy', 'setup-runner.sh');
    const script = await require('fs').promises.readFile(scriptPath, 'utf8');
    res.set('Content-Type', 'text/plain');
    res.send(script);
  } catch (e) {
    console.error('[deploy/linux] error', e);
    res.status(404).send('# Deploy script not found');
  }
});

app.get('/api/v1/runner/deploy/windows', async (req, res) => {
  try {
    const scriptPath = path.join(__dirname, '..', 'deploy', 'setup-runner.ps1');
    const script = await require('fs').promises.readFile(scriptPath, 'utf8');
    res.set('Content-Type', 'text/plain');
    res.send(script);
  } catch (e) {
    console.error('[deploy/windows] error', e);
    res.status(404).send('# Deploy script not found');
  }
});
